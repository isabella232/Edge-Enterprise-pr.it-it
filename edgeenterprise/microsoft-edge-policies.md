...
|[NewTabPageCompanyLogo](#newtabpagecompanylogo)|Imposta il logo aziendale della pagina nuova scheda (obsoleto)|
...
|[FetchKeepaliveDurationSecondsOnShutdown](#fetchkeepalivedurationsecondsonshutdown)|Recupera la durata di keepalive alla chiusura|
...
|[TabFreezingEnabled](#tabfreezingenabled)|Consenti blocco delle schede in background (obsoleto)|
...
Tieni presente che è ancora possibile usare [ ExtensionInstallForcelist](#extensioninstallforcelist) e [ExtensionInstallAllowlist](#extensioninstallallowlist) per consentire o forzare l'installazione di estensioni specifiche tramite JSON nell'esempio precedente, anche se lo Store è bloccato.
...
È possibile bloccare la modalità di sospensione dei singoli siti configurando il criterio [SleepingTabsBlockedForUrls](#sleepingtabsblockedforurls).
...
Le schede vengono messe automaticamente in modalità di sospensione quando il criterio [SleepingTabsEnabled](#sleepingtabsenabled) è abilitato o non è configurato e l'utente ha abilitato l'impostazione delle schede di sospensione.
...
```
  ...
  #### <a name="allow-microsoft-news-content-on-the-new-tab-page"></a>Consentire il contenuto di Microsoft News nella nuova scheda
...
Criteri correlati: [NewTabPageAllowedBackgroundTypes,](#newtabpageallowedbackgroundtypes) [NewTabPageQuickLinksEnabled](#newtabpagequicklinksenabled)
...
Criteri correlati: [NewTabPageAllowedBackgroundTypes,](#newtabpageallowedbackgroundtypes) [NewTabPageContentEnabled](#newtabpagecontentenabled)
...
I formati più avanzati potrebbero non essere ben supportati in alcune destinazioni e/o siti Web di incollamento. Di conseguenza, se questo criterio deve essere configurato, è consigliabile usare l'opzione URL non crittografato.
...
Se si disabilita o non si configura questo criterio, viene utilizzato il valore predefinito di 0 secondi e le richieste keepalive in sospeso verranno immediatamente annullate durante l'arresto del browser.
  ...
  - Nome univoco Criteri di gruppo: FetchKeepaliveDurationSecondsOnShutdown
  ...
  - Percorso (obbligatorio): SOFTWARE\Criteri\Microsoft\Edge
  - Percorso (consigliato): N/D
  - Nome valore: FetchKeepaliveDurationSecondsOnShutdown
  - Tipo valore: REG_DWORD
...
```
  ...
  - Nome chiave di preferenza: FetchKeepaliveDurationSecondsOnShutdown
  - Valore di esempio:
``` xml
<integer>1</integer>
```
...
Il criterio di riferimento predefinito di Microsoft Edge è stato rafforzato dal valore di no-referrer-when-downgrade a quello più sicuro strict-origin-when-cross-origin.

Quando questo criterio aziendale viene abilitato, il criterio di riferimento predefinito di Microsoft Edge verrà impostato sul valore precedente di no-referrer-when-downgrade.
...
- Se si abilita questo criterio, la finestra di dialogo di reindirizzamento occasionale e il banner non verranno mai mostrati agli utenti. I dati di esplorazione degli utenti non vengono importati quando si verifica il reindirizzamento.
...
Se si abilita questo criterio, verrà creato un profilo non rimovibile con l'account aziendale o dell'istituto di istruzione dell'utente in Windows. Questo profilo non può essere disconnesso o rimosso. Il profilo sarà non rimovibile solo se è connesso con un account locale o un account di Azure Active Directory che corrisponde all'account di accesso del sistema operativo.
...
Se questo criterio viene abilitato, il BHO non verrà installato. Se è già installato, verrà disinstallato nel prossimo aggiornamento Microsoft Edge.

Se i criteri non sono configurati o sono disabilitati, il BHO verrà installato.
...
Quando viene avviato Microsoft Edge per caricare un sito da IE, viene visualizzata una barra delle informazioni che spiega che il sito funziona al meglio in un browser moderno.
...
* Disable (0) = disabilita

* Sitelist (1) = ridirige i siti in base all'elenco dei siti incompatibili
...
```
  ...
  #### <a name="allow-freezing-of-background-tabs-obsolete"></a>Consenti blocco delle schede in background (obsoleto)

  
  >OBSOLETO: questo criterio è obsoleto e non funziona dopo Microsoft Edge 86.
  #### <a name="supported-versions"></a>Versioni supportate:

  - In Windows e macOS da 79, fino a 86

  #### <a name="description"></a>Descrizione

  Questo criterio non funziona, usa invece [SleepingTabsEnabled](#sleepingtabsenabled).
...
Se si disabilita questo criterio, non verrà bloccata nessuna scheda.
  ...
  - Nome Criteri di gruppo: Consenti blocco delle schede in background (obsoleto)
  - Percorso Criteri di gruppo (obbligatorio): modelli amministrativi/Microsoft Edge/
  - Percorso Criteri di gruppo (consigliato): N/D
  - Nome file ADMX Criteri di gruppo: MSEdge.admx
...
Se disabiliti i criteri, i popup con destinazione _blank sono autorizzati ad accedere, tramite JavaScript, alla pagina che ha richiesto di aprire il popup.
